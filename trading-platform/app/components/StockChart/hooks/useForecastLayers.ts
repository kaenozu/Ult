...
export const useForecastLayers = ({
  ...
  const [cache] = useState(() => new Map<number, GhostForecastCache>());

  // 2. AI Time Travel: Ghost Cloud (past prediction reproduction)
  // Refactored to useMemo with state-managed cache to avoid ref access during render
  const ghostForecastDatasets = useMemo(() => {
    ...
    const { targetArr, stopArr, color } = result;
    return [
      {
  ...
  }, [hoveredIdx, data, extendedData.labels.length, preCalculatedIndicators, cache]);
