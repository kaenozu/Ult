...
export const PerformanceDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      const snapshots = performanceMonitor.getSnapshots(100);

      const renderTimes = snapshots
        .filter(s => s.name.startsWith('render.'))
        .map(s => s.duration);

      const apiCallDurations = snapshots
      ...
      const avgApiTime = apiCallDurations.length > 0
        ? apiCallDurations.reduce((a, b) => a + b, 0) / apiCallDurations.length
        : 0;
      ...
      const latestCacheHitRate = cacheHitRates.length > 0
        ? cacheHitRates[cacheHitRates.length - 1]
        : 0;
