...
describe('Logic Monkey Test - Robustness under extreme conditions', () => {
  ...
  it('should not crash and keep values in range during a Flash Crash (90% drop)', () => {
    const data = generateRandomData(50, 1000, 0.05);
  ...
  it('should handle extreme price gaps (Penny Stock to Luxury Stock)', () => {
    ...
    const data = [
      ...generateRandomData(25, 0.1, 0.1),
      ...generateRandomData(25, 1000000, 0.1)
    ];
  ...
  it('should maintain logical consistency in mlPrediction ensemble', () => {
    for (let i = 0; i < 10; i++) { // Run 10 times with random noise
      const data = generateRandomData(60, 500, 0.1);
      ...
      expect(Number.isFinite(prediction.ensemblePrediction)).toBe(true);
    }
  });
});
