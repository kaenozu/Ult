...
if (!parsed.success) {
  ...
  // Security Check: Ensure default admin password is changed in production if admin is enabled
  if (parsed.data.NODE_ENV === 'production' && parsed.data.ENABLE_DEFAULT_ADMIN && parsed.data.DEFAULT_ADMIN_PASSWORD === 'admin123') {
    throw new Error('CRITICAL SECURITY ERROR: You have enabled default admin in production with the default password. Please change DEFAULT_ADMIN_PASSWORD.');
  }
}
