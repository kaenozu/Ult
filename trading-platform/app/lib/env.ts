...
if (!parsed.success) {
  ...
  // Security Check: Ensure production default admin password is changed if enabled
  if (
    parsed.data.NODE_ENV === 'production' &&
    parsed.data.ENABLE_DEFAULT_ADMIN &&
    parsed.data.DEFAULT_ADMIN_PASSWORD === DEFAULT_ADMIN_PASSWORD_VALUE
  ) {
    throw new Error('CRITICAL SECURITY ERROR: You have enabled default admin in production with the default password. Please change DEFAULT_ADMIN_PASSWORD or disable ENABLE_DEFAULT_ADMIN.');
  }
}
