...
describe('MarketRegimeDetector', () => {
  ...
  describe('detect', () => {
    ...
    it('should detect ranging market (ADX < 20)', () => {
      ...
      // RANGING or TRENDING with low ADX is acceptable for sideways data
      expect(['RANGING', 'TRENDING']).toContain(result.regime);
      expect(result.adx).toBeLessThanOrEqual(30);
    });
  ...
  describe('getRegimeDescription', () => {
    ...
    it('should handle high volatility trending', () => {
      const result = marketRegimeDetector.getRegimeDescription('TRENDING_UP', 'UP', 'HIGH');
