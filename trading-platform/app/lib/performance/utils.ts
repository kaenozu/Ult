...
export function logPerformanceReport(): void {
  ...
  report.operations
    ...
    .slice(0, 10)
    .forEach(op => {
      logger.info(`${op.name}: avg=${formatDuration(op.stats.avg)}, count=${op.stats.count}, max=${formatDuration(op.stats.max)}`);
    });
...
export function observeWebVitals(): void {
  ...
  new PerformanceObserver((list) => {
    ...
    entries.forEach(entry => {
      const perfEntry = entry as unknown as ExtendedPerformanceEntry;
