...
export class RiskManagementService {
   ...
   * 
   * @param order - 注文リクエスト（注意: このオブジェクトは直接変更されます）
   * @param portfolio - 現在のポートフォリオ状態
   * @param marketData - オプショナル: OHLCV市場データ（ATR計算用）
   * @returns リスク検証結果（調整された値を含む）
   * 
   * **重要**: この関数は入力された`order`パラメータを直接変更します。
   * 以下のプロパティが調整される可能性があります:
   * - `quantity`: ポジションサイジングにより調整
   * - `stopLoss`: 自動設定または調整
   * - `takeProfit`: 自動設定または調整
   * 
   * これにより、呼び出し元は調整済みの注文を直接使用できます。
   */
  validateOrder(
    order: OrderRequest,
    portfolio: Portfolio,
   ...
   * 
   * **Kelly基準について**:
   * ケリー基準は、資本成長を最大化する最適な賭け金を計算する数学的手法です。
   * 
   * **50%勝率の仮定**:
   * このシステムでは保守的に50%の勝率を仮定しています。理由:
   * 1. 新規トレーダーやシステムの初期段階では、過去の成績データが不十分
   * 2. 50%は統計的に中立的な値であり、過信を防ぐ
   * 3. 実際の勝率が50%を上回る場合、システムは保守的に動作（安全）
   * 4. 実際の勝率が50%を下回る場合でも、25%フラクションにより破産リスクを最小化
   * 
   * 将来的には、取引履歴から実際の勝率を計算し、動的に調整することを推奨します。
   * 
   * @private
   */
