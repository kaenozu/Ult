...
export default function MarketStatusCard() {
  ...
  const [vibe, setVibe] = useState<'neutral' | 'bull' | 'bear' | 'panic'>(
    'neutral'
  );
  ...
  return (
    <div
      className={`p-6 rounded-xl border transition-all duration-500 ${vibeStyles[vibe]} min-h-[200px] flex flex-col justify-between relative overflow-hidden group`}
    >
      {/* Vibe Background Effect for Bull/Panic */}
      {vibe === 'bull' && (
        <div className='absolute inset-0 bg-emerald-500/5 blur-3xl -z-10 group-hover:bg-emerald-500/10 transition-colors' />
      )}
      {vibe === 'panic' && (
        <div className='absolute inset-0 bg-rose-500/10 blur-xl -z-10 animate-pulse' />
      )}

      <div className='flex justify-between items-start'>
        <div className='flex items-center gap-2'>
          <Activity
            className={`w-5 h-5 ${vibe === 'panic' ? 'animate-bounce text-rose-500' : 'text-muted-foreground'}`}
          />
      ...
      <div className='text-center py-4 z-10'>
        <div
          className={`text-3xl md:text-5xl mb-2 transition-all duration-300 ${textStyles[vibe]}`}
        >
          {regime}
        </div>
