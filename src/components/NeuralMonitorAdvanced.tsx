...
const SafetyHeader = ({ status }: { status: "SAFE" | "TRIPPED" | "UNKNOWN" }) => {
    return (
        <div
            className={`flex items-center justify-between px-4 py-2 border-b-2 font-mono text-sm uppercase tracking-widest ${status === "SAFE"
    ...
    return (
        <div className="py-1 px-2 font-mono text-xs border-l-2 border-slate-700/50 hover:bg-slate-800/30 transition-colors">
            <span className="text-slate-500 mr-2">
                [{new Date(entry.timestamp).toLocaleTimeString([], { hour12: false, fractionalSecondDigits: 3 })}]
            </span>
...
export default function NeuralMonitor() {
    ...
    const [safetyStatus, setSafetyStatus] = useState<"SAFE" | "TRIPPED" | "UNKNOWN">("UNKNOWN");
    ...
    useEffect(() => {
        ...
        onMessage<ApprovalResponseMessage>("approval_response", (msg) => {
            ...
            if (approvalRequest && msg.payload.request_id === approvalRequest.request_id) {
                                ...
                                {actions.map((action) => (
                                    ...
                                    >
                                        <div className="flex justify-between items-center mb-1">
                                            <span className={`text-xs font-bold px-1 rounded ${action.content.type === "BUY" ? "bg-emerald-500/20 text-emerald-400" :
                                                action.content.type === "SELL" ? "bg-red-500/20 text-red-400" :
                                                    "bg-slate-700 text-slate-300"
                                                }`}>
                                                {action.content.type}
                                            </span>
                                            <span className="text-xs font-mono text-slate-400">{action.content.ticker}</span>
                                        </div>
