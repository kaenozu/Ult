...
このモジュールは、株価データと取引戦略に基づいて取引シミュレーションを行う機能を提供します。
...
class TradeSimulator:
    """取引シミュレーションを行うクラス（単一責任：取引シミュレーションの実行）"""
    ...
    def run_simulation(
        ...
        entry_prices: Dict[str, float] = {t: 0.0 for t in data}
        ...
        for i in range(len(full_index) - 1):
            ...
            for ticker, df in aligned_data.items():
                ...
                if isinstance(today_sig, Order):
                    exit_executed = self._process_order_signals(
                        today_sig,
                        ticker,
                        holdings,
                        entry_prices,
    ...
    def _create_trade_record(self, ticker, reason, entry, exec_price, trade_type, date):
        """取引記録作成"""
        return {
            "ticker": ticker,
            "entry_date": None,
    ...
    def _process_new_positions(
        ...
        if holdings[ticker] == 0 and today_sig == 1:
            ...
            holdings[ticker] = -shares
            entry_prices[ticker] = exec_price
        return holdings, cash, entry_prices, highest_prices, trailing_stop_levels

    def _get_position_state(self, holdings):
        """ポジション状態取得"""
        position_state = 0
