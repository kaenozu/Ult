...
class AutoTrader:
    ...
    def __init__(self, paper_trader: PaperTrader):
        ...
        # Configuration
        self.max_budget_per_trade = 50000.0  # ¥50,000 per entry
        self.max_total_invested = 200000.0   # ¥200,000 max portfolio
    ...
    def _monitor_positions(self):
        ...
        for _, pos in positions.iterrows():
            ...
            if pnl_pct <= -self.stop_loss_pct:
                logger.info(f"AutoTrader: Stop Loss triggered for {ticker} ({pnl_pct:.1%})")
                self.pt.execute_trade(ticker, "SELL", int(qty), current_price, reason="Stop Loss (Auto)")
            
            # Take Profit check (+10% or explicit target)
            elif pnl_pct >= 0.10:
                logger.info(f"AutoTrader: Take Profit triggered for {ticker} ({pnl_pct:.1%})")
                self.pt.execute_trade(ticker, "SELL", int(qty), current_price, reason="Take Profit (Auto)")
    ...
    def _analyze_ticker(self, ticker: str) -> int:
        ...
        """
        data_map = fetch_stock_data([ticker], period="1y") # Need enough for LightGBM? 5y is better?
    ...
    def _execute_buy(self, ticker: str):
        ...
        if cost > self.max_budget_per_trade:
            ...
            logger.info(f"Skipping {ticker}: Min lot cost ¥{cost:,.0f} > Budget ¥{self.max_budget_per_trade:,.0f}")
            return

        logger.info(f"AutoTrader: Executing BUY for {ticker} x {quantity}")
        self.pt.execute_trade(ticker, "BUY", quantity, price, reason="Auto-Pilot Entry", strategy="Auto-Consensus")
