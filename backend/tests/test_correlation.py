...
class TestMarketCorrelation:
    ...
    def test_calculate_correlation_zero_std(self):
        """Test correlation with zero standard deviation (line 43)"""
        analyzer = MarketCorrelation()

        # All same values - zero standard deviation
        stock_prices = [100, 100, 100, 100, 100]
        index_prices = [1000, 1000, 1000, 1000, 1000]

        correlation = analyzer.calculate_correlation(stock_prices, index_prices)
        assert correlation == 0.0

    def test_calculate_beta_mismatched_lengths(self):
        """Test beta calculation with mismatched lengths (line 58)"""
        analyzer = MarketCorrelation()

        with pytest.raises(ValueError, match="same length"):
            analyzer.calculate_beta([100, 102, 104], [1000, 1020])

    def test_calculate_beta_insufficient_data(self):
        """Test beta calculation with insufficient data (line 60)"""
        analyzer = MarketCorrelation()

        with pytest.raises(ValueError, match="at least 2"):
            analyzer.calculate_beta([100], [1000])

    def test_calculate_beta_returns_calculation_error(self):
        """Test beta calculation when returns calculation fails (line 69)"""
        analyzer = MarketCorrelation()

        # This test checks the edge case where returns calculation might fail
        # The implementation should handle this gracefully
        stock_prices = [100.0, 102.0, 104.0]
        index_prices = [100.0, 101.0, 102.0]

        # Should not raise an error for valid data
        beta = analyzer.calculate_beta(stock_prices, index_prices)
        assert isinstance(beta, float)

    def test_calculate_beta_single_return(self):
        """Test beta calculation with single return (line 73)"""
        analyzer = MarketCorrelation()

        # Only 2 data points = 1 return
        stock_prices = [100.0, 102.0]
    ...
    def test_calculate_beta_zero_variance(self):
        """Test beta calculation with zero variance (line 86)"""
        analyzer = MarketCorrelation()

        # Index prices with no variance
        stock_prices = [100.0, 102.0, 104.0, 106.0]
        index_prices = [100.0, 100.0, 100.0, 100.0]

        beta = analyzer.calculate_beta(stock_prices, index_prices)
        assert beta == 1.0

    def test_detect_trend_insufficient_data(self):
        """Test trend detection with insufficient data (line 100)"""
        analyzer = MarketCorrelation()
    ...
    def test_detect_trend_zero_denominator(self):
        """Test trend detection with zero denominator (line 116)"""
        analyzer = MarketCorrelation()

        # Single data point causes zero denominator
        prices = [100]
        trend = analyzer.detect_trend(prices)

        assert trend == MarketTrend.NEUTRAL

    def test_detect_trend_zero_avg_price(self):
        """Test trend detection with zero average price (line 123)"""
        analyzer = MarketCorrelation()
    ...
    def test_composite_signal_bearish_market_buy_low_correlation(self):
        """Test composite signal with bearish market and buy signal (low correlation) (lines 181-183)"""
        analyzer = MarketCorrelation()
        ...
        assert result["recommendation"] == "cautious_buy"
        assert result["confidence"] == "low"
        assert "low correlation" in result["reasoning"].lower()

    def test_composite_signal_bearish_market_sell_low_correlation(self):
        """Test composite signal with bearish market and sell signal (low correlation) (lines 193-195)"""
        analyzer = MarketCorrelation()
    ...
    def test_composite_signal_bullish_market_sell_high_correlation(self):
        """Test composite signal with bullish market and sell signal (high correlation) (lines 202-204)"""
        analyzer = MarketCorrelation()
        ...
        assert result["confidence"] == "low"
        assert "high correlation" in result["reasoning"].lower()

    def test_composite_signal_bullish_market_sell_low_correlation(self):
        """Test composite signal with bullish market and sell signal (low correlation) (lines 210-212)"""
        analyzer = MarketCorrelation()
