...
class TestTradeJournalAnalyzer:
    ...
    def test_calculate_win_rate_no_closed_trades(self):
        """Test win rate calculation with no closed trades (line 37)"""
        analyzer = TradeJournalAnalyzer()

        # Add only open trades
        for i in range(5):
            analyzer.add_entry(self._create_entry(status="OPEN"))

        win_rate = analyzer.calculate_win_rate()

        assert win_rate == 0.0

    def test_get_time_span_empty_entries(self):
        """Test time span calculation with empty entries (line 174)"""
        analyzer = TradeJournalAnalyzer()

        time_span = analyzer._get_time_span()

        assert time_span == timedelta(0)

    def test_find_loss_sequences_multiple_sequences(self):
        """Test finding multiple loss sequences (lines 205-207)"""
        analyzer = TradeJournalAnalyzer()

        base_time = datetime(2026, 1, 25, 10, 0, 0)

        # First sequence: 3 losses within 30 minutes
        for i in range(3):
            analyzer.add_entry(self._create_entry(
        ...
        # Gap of 1 hour
        # Second sequence: 2 losses within 30 minutes
        for i in range(2):
            analyzer.add_entry(self._create_entry(
        ...
        sequences = analyzer._find_loss_sequences()

        assert len(sequences) == 2
        assert len(sequences[0]) == 3
        assert len(sequences[1]) == 2

    def test_find_loss_sequences_single_loss(self):
        """Test that single loss is not counted as sequence (line 213)"""
        analyzer = TradeJournalAnalyzer()
    ...
    def test_find_loss_sequences_winning_trade_breaks_sequence(self):
        """Test that winning trade breaks loss sequence (lines 248-251)"""
        analyzer = TradeJournalAnalyzer()
        ...
        # Should have one sequence of 2 losses (the winning trade breaks it)
        assert len(sequences) == 1
        assert len(sequences[0]) == 2

    def test_extract_patterns_by_hour_morning(self):
        """Test extracting patterns by hour for morning period (lines 246-248)"""
        analyzer = TradeJournalAnalyzer()

        # Add trades at 9am (morning)
        for i in range(5):
            entry = self._create_entry(
                timestamp=datetime(2026, 1, 25, 9, 10 + i * 5, 0),
                profit=2000,
                symbol="AAPL",
                status="CLOSED",
                profit_percent=2.0
        ...
        # Should find morning pattern
        morning_pattern = next((p for p in patterns if "Morning" in p.description), None)
        assert morning_pattern is not None
        assert "09:00" in morning_pattern.description

    def test_extract_patterns_by_hour_afternoon(self):
        """Test extracting patterns by hour for afternoon period (lines 248-250)"""
        analyzer = TradeJournalAnalyzer()

        # Add trades at 2pm (afternoon)
        for i in range(5):
            entry = self._create_entry(
                timestamp=datetime(2026, 1, 25, 14, 10 + i * 5, 0),
                profit=2000,
                symbol="GOOGL",
                status="CLOSED",
                profit_percent=2.0
        ...
        # Should find afternoon pattern
        afternoon_pattern = next((p for p in patterns if "Afternoon" in p.description), None)
        assert afternoon_pattern is not None
        assert "14:00" in afternoon_pattern.description

    def test_extract_patterns_by_hour_evening(self):
        """Test extracting patterns by hour for evening period (lines 250-251)"""
        analyzer = TradeJournalAnalyzer()

        # Add trades at 7pm (evening)
        for i in range(5):
            entry = self._create_entry(
                timestamp=datetime(2026, 1, 25, 19, 10 + i * 5, 0),
                profit=2000,
                symbol="TSLA",
                status="CLOSED",
                profit_percent=2.0
        ...
        # Should find evening pattern
        evening_pattern = next((p for p in patterns if "Evening" in p.description), None)
        assert evening_pattern is not None
        assert "19:00" in evening_pattern.description
