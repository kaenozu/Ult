...
## 2026-03-12 - Math/Array Loop Optimizations (Float64Array vs Standard Arrays)
**Learning:** While `Float64Array` might intuitively seem faster for numerical operations, allocating a `new Float64Array(prices)` inside hot functions like `calculateSMA` or `calculateEMA` introduces significant copying/allocation overhead in V8. Additionally, `new Array(length).fill(NaN)` creates objects that add unnecessary allocation time; pre-allocating an array and iterating via a simple `for (let i = 0; i < length; i++) result[i] = NaN` is actually much faster. Replacing division by period (`/ period`) with inverse multiplication (`* invPeriod = 1 / period`), and swapping `isNaN(val)` with `val === val`, yields massive micro-optimization gains (~50%+ performance improvement in hot loops).
**Action:** In frequently called numerical loops (e.g. data pipelines, indicator calculations), avoid instantiating new typed arrays from standard arrays if it means copying. Avoid `.fill(NaN)` inside fast paths; use a manual setup loop instead. Pre-compute constants like `1 / value` when dividing inside loops.
