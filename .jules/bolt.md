...
## 2026-03-01 - Math.max Performance in Tight Loops\n**Learning:** Using `Math.max` and `Math.abs` in inner loops over large arrays (like in Technical Analysis Indicators) introduces measurable function call overhead. Replacing these with inline ternaries (e.g. `variance > 0 ? variance : 0`) or direct logic can reduce execution time noticeably (e.g. 15%+ speedup for Bollinger Bands).\n**Action:** When optimizing tight numerical loops, evaluate replacing built-in `Math` methods and generic validation wrappers (e.g. `_getValidPrice`) with inline boolean checks and ternary logic.

## 2026-03-05 - Math.max Performance in Tight Loops
**Learning:** Using `Math.max` and `Math.abs` in inner loops over large arrays (like in Technical Analysis Indicators) introduces measurable function call overhead. Replacing these with inline ternaries (e.g. `variance > 0 ? variance : 0`) or direct logic can reduce execution time noticeably (e.g. 15%+ speedup for Bollinger Bands).
**Action:** When optimizing tight numerical loops, evaluate replacing built-in `Math` methods and generic validation wrappers (e.g. `_getValidPrice`) with inline boolean checks and ternary logic.
