...
## 2026-03-15 - [V8 Array Allocation Overhead]
**Learning:** In heavily used computational utility files (like `calculations.ts` for financial indicators), using `Array.push()` and `Array.slice()` inside loops creates excessive memory allocation and garbage collection overhead in V8/Node.js, degrading performance significantly.
**Action:** Always prefer pre-allocating arrays with `new Array(length)` or initializing with fixed sizes and using direct index assignment (`arr[i] = value`) for numerical calculations. Single-pass iteration logic without creating intermediate arrays can yield 50-80% execution speed improvements.
