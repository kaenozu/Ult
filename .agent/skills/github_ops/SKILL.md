...
2.  **Create PR**:
    ...
    ```
    _Interactive mode (simpler)_: `gh pr create --web` (opens browser) or just `gh pr create` (prompts in terminal). For automation, use flags.
...
- **Descriptive Titles**: Use titles that explain _what_ changed (e.g., "feat: Add Auto-Pilot UI", "fix: Resolve build error in api.ts").
